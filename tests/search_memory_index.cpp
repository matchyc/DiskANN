...
int search_memory_index(diskann::Metric& metric, const std::string& index_path,
                        ...
                        const float fail_if_recall_below, std::string& evaluation_save_file) {
  // Load the query file
  T*        query = nullptr;
  ...
  if (!evaluation_save_file.empty()) {
    evaluation_ofs.open(evaluation_save_file, std::ios::out);
  }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    if (evaluation_ofs.is_open()) {
      evaluation_ofs << L << "," << displayed_qps << "," << avg_cmps << ","
                     << mean_latency << "," << best_recall / 100;
      evaluation_ofs << std::endl;
    }
    std::cout << std::endl;
  }
    if (evaluation_ofs.is_open()) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    desc.add_options()("evaluation_save_path",
                        po::value<std::string>(&evaluation_save_file)->default_value(std::string("null")), "File to save evaluation results (recall, qps, etc.)");
  ...
  try {
    if (data_type == std::string("int8")) {
      return search_memory_index<int8_t>(
          ...
          show_qps_per_thread, fail_if_recall_below, evaluation_save_file);
    }
...
//                         const std::vector<unsigned>& Lvec, const bool dynamic,
//                         const bool tags, const bool flags, std::string& evaluation_save_file) {
//   // Load the query file
//   T*        query = nullptr;
//   unsigned* gt_ids = nullptr;
//   float*    gt_dists = nullptr;
...
//         (float) query_num;
//     if (evaluation_ofs.is_open()) {
//       evaluation_ofs << L << "," << qps << "," << avg_cmps << ","
//                      << mean_latency << "," << recall / 100;
//       evaluation_ofs << std::endl;
//     }
//     if (tags) {
...
//                                          Lvec, dynamic, tags, flags, evaluation_save_file);
//     }

//     else if (data_type == std::string("uint8")) {
//       return search_memory_index<uint8_t>(
//           metric, index_path_prefix, result_path, query_file, gt_file,
//           num_threads, K, Lvec, dynamic, tags, flags, evaluation_save_file);
//     } else if (data_type == std::string("float")) {
